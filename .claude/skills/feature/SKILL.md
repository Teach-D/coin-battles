...
**Critical 이슈가 있는 경우:**
리뷰 결과(파일명, 라인, 수정 이유)를 정리해 `backend-dev-agent`에게 전달하고 수정을 위임합니다.
메인 세션에서 직접 코드를 수정하지 않습니다.

`backend-dev-agent` 프롬프트 형식:
```
다음 리뷰 지적 사항을 수정해주세요.
[Critical 이슈 목록 — 파일 경로, 라인, 수정 내용]
수정 후 ./gradlew test 를 실행해 테스트가 통과하는지 확인하세요.
```

backend-dev-agent 완료 후 Stage 4를 재실행합니다 (최대 2회).
