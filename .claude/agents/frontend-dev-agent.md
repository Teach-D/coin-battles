...
**웹페이지, 섹션, 컴포넌트를 디자인하거나 UI를 구현할 때는 반드시 `coinbattle-design` 스킬을 먼저 호출해야 한다.**

- 페이지/컴포넌트 신규 구현 요청 → Skill 툴로 `coinbattle-design` 호출
- UI 레이아웃, 스타일, 색상, 컴포넌트 구조가 포함된 모든 작업에 적용
- 스킬 호출 없이 직접 JSX/TSX를 작성하지 않는다

```
// 올바른 순서
1. Skill("coinbattle-design", "구현할 화면 설명")
2. 스킬이 반환한 디자인 가이드에 따라 코드 구현
```
...
2. **디자인 스킬 호출**: UI가 포함된 작업이면 `coinbattle-design` 스킬을 반드시 먼저 호출
3. **컴포넌트 설계**: 페이지/컴포넌트/훅 분리 구조 확정
4. **구현**: 타입 정의 → 훅 → 컴포넌트 순서로 구현
5. **검증**: TypeScript 오류, 빌드 성공 여부 확인
6. **문서화**: 필요 시 설계 결정 사항 정리 (한글로 작성)
