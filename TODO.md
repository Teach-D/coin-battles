...
- [ ] `UserController.kt` `PATCH /api/users/me` 추가 — 닉네임 변경 엔드포인트 (`UpdateProfileRequest.kt` DTO 포함, 닉네임 중복 체크)
- [ ] `GET /api/users/me/stats` 추가 — PVP 전적(승/패/승률), 시즌 최고 수익률 반환 (`UserStatsResponse.kt`)

### Frontend — 사용자 프로필 페이지

- [ ] `ProfilePage.tsx` 생성 — 닉네임 수정 폼, PVP 전적(승/패/승률), 시즌 최고 수익률 표시
- [ ] `App.tsx` `/profile` 라우트 추가 + 상단 네비게이션에 프로필 링크 연결
